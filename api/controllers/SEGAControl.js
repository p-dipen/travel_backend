...
const SEGAController = () => {
  const register = async (req, res) => {
    const { query } = req;
    const { table } = query;
    let pushTable = [];
    if (table) {
      pushTable = table.split(",");
    }
    try {
      const currentStampdata = await syncApi().currentTimeStamp("currentstamp");
      const token = await syncApi().commoncall(
        "http://segatours.toursupport.ru/xmlgate/export/default.php?samo_action=auth",
        {
          login: "PraivitXML",
          passwordDigest:
            "BHHwlT8jhS6n%2bsCzjJePrY7HZj%2fOJg35i6ky926jX8x8UEa%2fpRhKJucastwpDuCU",
        }
      );
      const tokenUsed = token.Result[0].$.partner_token;
      let stamp;
      if (pushTable.indexOf("state") > -1 || pushTable.length == 0) {
        let stateSchema = [];
        do {
          stamp = "";
          stamp = await State.max("stamp", {
            where: { status: { [Op.or]: ["", " ", null] } },
          });
          if (stamp === null || stamp === 0) {
            stamp = "0x0000000000000000";
          }
          const state = await syncApi().callApi(
            "state",
            stamp,
            currentStampdata
          );
          ...
          if (state.state) {
            ...
            await State.bulkCreate(stateSchema, {
              updateOnDuplicate: ["status", "name", "lname", "alias", "stamp"],
            }).then(() => {
      ...
      // if (false) {
      if (pushTable.indexOf("region") > -1 || pushTable.length == 0) {
        // region
        let regionSchema = [];
        do {
          stamp = "";
          stamp = await Region.max("stamp", {
            where: { status: { [Op.or]: ["", " ", null] } },
          });
          if (stamp === null || stamp === 0) {
            stamp = "0x0000000000000000";
          }
          const region = await syncApi().callApi(
            "region",
            stamp,
            currentStampdata
          );
          ...
          if (region.region) {
            ...
            await Region.bulkCreate(regionSchema, {
              updateOnDuplicate: [
                "stateid",
                "lname",
                "name",
      ...
      if (pushTable.indexOf("town") > -1 || pushTable.length == 0) {
        // town
        let townSchema = [];
        do {
          stamp = "";
          stamp = await Town.max("stamp", {
            where: { status: { [Op.or]: ["", " ", null] } },
          });
          console.log(stamp);
          if (stamp === null || stamp === 0) {
          ...
          if (town.town) {
            ...
            await Town.bulkCreate(townSchema, {
              updateOnDuplicate: [
                "name",
                "stateid",
                "lname",
      ...
      if (pushTable.indexOf("star") > -1 || pushTable.length == 0) {
        // star
        let starSchema = [];
        do {
          stamp = "";
          stamp = await Star.max("stamp", {
            where: { status: { [Op.or]: ["", " ", null] } },
          });
          if (stamp === null || stamp === 0) {
            stamp = "0x0000000000000000";
          ...
          if (star.star) {
            ...
            await Star.bulkCreate(starSchema, {
              updateOnDuplicate: ["stamp", "lname", "name", "status"],
            }).then(() => {
      ...
      if (pushTable.indexOf("hotel") > -1 || pushTable.length == 0) {
        // hotel
        let hotelSchema = [];
        do {
          stamp = "";
          stamp = await Hotel.max("stamp", {
            where: { status: { [Op.or]: ["", " ", null] } },
          });
          if (stamp === null || stamp === 0) {
            stamp = "0x0000000000000000";
          }
          const hotel = await syncApi().callApi(
            "hotel",
            stamp,
            currentStampdata
          );
          ...
          if (hotel.hotel) {
            ...
            await Hotel.bulkCreate(hotelSchema, {
              updateOnDuplicate: [
                "status",
                "name",
                "lname",
      ...
      if (pushTable.indexOf("room") > -1 || pushTable.length == 0) {
        // room
        let roomSchema = [];
        do {
          stamp = "";
          stamp = await Room.max("stamp", {
            where: { status: { [Op.or]: ["", " ", null] } },
          });
          if (stamp === null || stamp === 0) {
            stamp = "0x0000000000000000";
          ...
          if (room.room) {
            ...
            await Room.bulkCreate(roomSchema, {
              updateOnDuplicate: ["lname", "name", "status", "stamp"],
            }).then(() => {
      ...
      if (pushTable.indexOf("allocation") > -1 || pushTable.length == 0) {
        // allocation
        let allocationSchema = [];
        do {
          stamp = "";
          stamp = await Allocation.max("stamp", {
            where: { status: { [Op.or]: ["", " ", null] } },
          });
          if (stamp === null || stamp === 0) {
            stamp = "0x0000000000000000";
          }
          const allocation = await syncApi().callApi(
            "htplace",
            stamp,
            currentStampdata
          );
          ...
          if (allocation.htplace) {
            ...
            await Allocation.bulkCreate(allocationSchema, {
              updateOnDuplicate: [
                "stamp",
                "lname",
                "name",
      ...
      if (pushTable.indexOf("meal") > -1 || pushTable.length == 0) {
        // Meal
        let mealSchema = [];
        do {
          stamp = "";
          stamp = await Meal.max("stamp", {
            where: { status: { [Op.or]: ["", " ", null] } },
          });
          if (stamp === null || stamp === 0) {
            stamp = "0x0000000000000000";
          ...
          if (meal.meal) {
            ...
            await Meal.bulkCreate(mealSchema, {
              updateOnDuplicate: ["status", "name", "lname", "stamp"],
            }).then(() => {
      ...
      if (pushTable.indexOf("tour") > -1 || pushTable.length == 0) {
        // tour
        let tourSchema = [];
        do {
          stamp = "";
          stamp = await Tour.max("stamp", {
            where: { status: { [Op.or]: ["", " ", null] } },
          });
          if (stamp === null || stamp === 0) {
            stamp = "0x0000000000000000";
          ...
          if (tour.tour) {
            ...
            await Tour.bulkCreate(tourSchema, {
              updateOnDuplicate: ["stamp", "status", "name", "lname"],
            }).then(() => {
      ...
      if (pushTable.indexOf("port") > -1 || pushTable.length == 0) {
        // port
        let portSchema = [];
        do {
          stamp = "";
          stamp = await Port.max("stamp", {
            where: { status: { [Op.or]: ["", " ", null] } },
          });
          if (stamp === null || stamp === 0) {
            stamp = "0x0000000000000000";
          ...
          if (port.port) {
            ...
            await Port.bulkCreate(portSchema, {
              updateOnDuplicate: ["townid", "stamp", "status", "name", "lname"],
            }).then(() => {
      ...
      if (pushTable.indexOf("freight") > -1 || pushTable.length == 0) {
        // freight
        let freightSchema = [];
        do {
          stamp = "";
          stamp = await Freight.max("stamp", {
            where: { status: { [Op.or]: ["", " ", null] } },
          });
          if (stamp === null || stamp === 0) {
            stamp = "0x0000000000000000";
          }
          const freight = await syncApi().callApi(
            "freight",
            stamp,
            currentStampdata
          );
          ...
          if (freight.freight) {
            ...
            await Freight.bulkCreate(freightSchema, {
              updateOnDuplicate: [
                "trgportid",
                "srcportid",
                "targetid",
      ...
      if (pushTable.indexOf("class") > -1 || pushTable.length == 0) {
        // class
        let classSchema = [];
        do {
          stamp = "";
          stamp = await Class.max("stamp", {
            where: { status: { [Op.or]: ["", " ", null] } },
          });
          if (stamp === null || stamp === 0) {
            stamp = "0x0000000000000000";
          }
          const classC = await syncApi().callApi(
            "class",
            stamp,
            currentStampdata
          );
          ...
          if (classC.class) {
            ...
            await Class.bulkCreate(classSchema, {
              updateOnDuplicate: ["stamp", "lname", "name", "status"],
            }).then(() => {
      ...
      if (pushTable.indexOf("stopsale") > -1 || pushTable.length == 0) {
        // Stopsale
        let stopsaleSchema = [];
        do {
          stamp = "";
          stamp = await Stopsale.max("stamp", {
            where: { status: { [Op.or]: ["", " ", null] } },
          });
          if (stamp === null || stamp === 0) {
            stamp = "0x0000000000000000";
          }
          const stopsale = await syncApi().callApi(
            "stopsale",
            stamp,
            currentStampdata,
            tokenUsed
          ...
          if (stopsale.stopsale) {
            ...
            await Stopsale.bulkCreate(stopsaleSchema, {
              updateOnDuplicate: [
                "htplaceid",
                "hotelid",
                "dateend",
      ...
      if (pushTable.indexOf("release") > -1 || pushTable.length == 0) {
        // release
        let releaseSchema = [];
        do {
          stamp = "";
          stamp = await Release.max("stamp", {
            where: { status: { [Op.or]: ["", " ", null] } },
          });
          if (stamp === null || stamp === 0) {
            stamp = "0x0000000000000000";
          }
          const release = await syncApi().callApi(
            "release",
            stamp,
            currentStampdata,
            tokenUsed
          ...
          if (release.release) {
            ...
            await Release.bulkCreate(releaseSchema, {
              updateOnDuplicate: [
                "hotelid",
                "dateeng",
                "datebeg",
      ...
      if (pushTable.indexOf("servtype") > -1 || pushTable.length == 0) {
        // servtype
        let servtypeSchema = [];
        do {
          stamp = "";
          stamp = await Servtype.max("stamp", {
            where: { status: { [Op.or]: ["", " ", null] } },
          });
          if (stamp === null || stamp === 0) {
            stamp = "0x0000000000000000";
          }
          const servtype = await syncApi().callApi(
            "servtype",
            stamp,
            currentStampdata,
            tokenUsed
          ...
          if (servtype.servtype) {
            ...
            await Servtype.bulkCreate(servtypeSchema, {
              updateOnDuplicate: ["name", "lname", "stamp", "status"],
            }).then(() => {
      ...
      if (pushTable.indexOf("service") > -1 || pushTable.length == 0) {
        // service
        let serviceSchema = [];
        do {
          stamp = "";
          stamp = await Service.max("stamp", {
            where: { status: { [Op.or]: ["", " ", null] } },
          });
          if (stamp === null || stamp === 0) {
            stamp = "0x0000000000000000";
          }
          const service = await syncApi().callApi(
            "service",
            stamp,
            currentStampdata,
            tokenUsed
          ...
          if (service.service) {
            ...
            await Service.bulkCreate(serviceSchema, {
              updateOnDuplicate: [
                "name",
                "lname",
                "servtypeid",
      ...
      if (pushTable.indexOf("spos") > -1 || pushTable.length == 0) {
        // spos
        let sposSchema = [];
        do {
          stamp = "";
          stamp = await Spos.max("stamp", {
            where: { status: { [Op.or]: ["", " ", null] } },
          });
          if (stamp === null || stamp === 0) {
            stamp = "0x0000000000000000";
          }
          const spos = await syncApi().callApi(
            "spos",
            stamp,
            currentStampdata,
            tokenUsed
      ...
      if (pushTable.indexOf("currency") > -1 || pushTable.length == 0) {
        // currency
        let currencySchema = [];
        do {
          stamp = "";
          stamp = await Currency.max("stamp", {
            where: { status: { [Op.or]: ["", " ", null] } },
          });
          if (stamp === null || stamp === 0) {
            stamp = "0x0000000000000000";
          }
          const currency = await syncApi().callApi(
            "currency",
            stamp,
            currentStampdata,
            tokenUsed
          ...
          if (currency.currency) {
            ...
            await Currency.bulkCreate(currencySchema, {
              updateOnDuplicate: ["alias", "name", "status", "lname", "stamp"],
            }).then(() => {
      ...
      if (pushTable.indexOf("hotelsalepr") > -1 || pushTable.length == 0) {
        // hotelsalepr
        // if(false) {
        ...
        do {
          stamp = "";
          stamp = await Hotelsalepr.max("stamp", {
            where: { status: { [Op.or]: ["", " ", null] } },
          });
          if (stamp === null || stamp === 0) {
            stamp = "0x0000000000000000";
          }
          const hotelsalepr = await syncApi().callApi(
            "hotelsalepr",
            stamp,
            currentStampdata,
            tokenUsed
          ...
          hotelsaleprSchema = [];
          if (hotelsalepr.hprice[0].$.inc != undefined) {
            hotelsalepr.hprice.forEach((element) => {
              ...
              element.$.discountmoney =
                parseFloat(element.$.discountmoney) || null;
              element.$.useascheckin =
                parseInt(element.$.useascheckin, 10) || null;
              element.$.rqdaysfrom = parseInt(element.$.rqdaysfrom, 10) || null;
              element.$.rqdaysstill =
                parseInt(element.$.rqdaysstill, 10) || null;
              ...
              element.$.rnights_from =
                parseInt(element.$.rnights_from, 10) || null;
              element.$.rnights_till =
                parseInt(element.$.rnights_till, 10) || null;
              element.$.rnights_free =
                parseInt(element.$.rnights_free, 10) || null;
              element.$.status = element.$.status.trim();
            ...
            await Hotelsalepr.bulkCreate(hotelsaleprSchema, {
              updateOnDuplicate: [
                "useascheckin",
                "discountmoney",
                "discount",
          ...
          console.log(hotelsaleprSchema.length);
        } while (hotelsaleprSchema.length >= 500);
      }
      if (pushTable.indexOf("servicesalepr") > -1 || pushTable.length == 0) {
        // }
        // servicesalepr
        let servicesaleprSchema = [];
        do {
          stamp = "";
          stamp = await Servicesalepr.max("stamp", {
            where: { status: { [Op.or]: ["", " ", null] } },
          });
          if (stamp === null || stamp === 0) {
            stamp = "0x0000000000000000";
          }
          const servicesalepr = await syncApi().callApi(
            "servicesalepr",
            stamp,
            currentStampdata,
            tokenUsed
          ...
          if (servicesalepr.sprice) {
            servicesalepr.sprice.forEach((element) => {
              ...
              element.$.grouphotelid =
                parseInt(element.$.grouphotel, 10) || null;
            ...
            await Servicesalepr.bulkCreate(servicesaleprSchema, {
              updateOnDuplicate: [
                "hotelid",
                "status",
                "stamp",
      ...
      if (pushTable.indexOf("reserve") > -1 || pushTable.length == 0) {
        // reserve
        let reserveSchema = [];
        do {
          stamp = "";
          stamp = await Reserve.max("stamp", {
            where: { status: { [Op.or]: ["", " ", null] } },
          });
          if (stamp === null || stamp === 0) {
            stamp = "0x0000000000000000";
          }
          const reserve = await syncApi().callApi(
            "reserve",
            stamp,
            currentStampdata,
            tokenUsed
          ...
          if (reserve.reserve) {
            reserve.reserve.forEach((element) => {
              ...
              element.$.commitmentpart =
                parseInt(element.$.commitmentpart, 10) || null;
              element.$.releasedays =
                parseInt(element.$.releasedays, 10) || null;
              element.$.status = element.$.status.trim();
              reserveSchema.push(element.$);
            });
            await Reserve.bulkCreate(reserveSchema, {
              updateOnDuplicate: [
                "datebeg",
                "stamp",
                "releasedays",
      ...
      if (pushTable.indexOf("alloctype") > -1 || pushTable.length == 0) {
        // alloctype
        let alloctypeSchema = [];
        do {
          stamp = "";
          stamp = await Alloctype.max("stamp", {
            where: { status: { [Op.or]: ["", " ", null] } },
          });
          if (stamp === null || stamp === 0) {
            stamp = "0x0000000000000000";
          }
          const alloctype = await syncApi().callApi(
            "alloctype",
            stamp,
            currentStampdata,
            tokenUsed
          ...
          if (alloctype.alloctype) {
            alloctype.alloctype.forEach((element) => {
              ...
              element.$.autoconfirm =
                parseInt(element.$.autoconfirm, 10) || null;
              element.$.resorder = parseInt(element.$.resorder, 10) || null;
              element.$.status = element.$.status.trim();
              alloctypeSchema.push(element.$);
            });
            await Alloctype.bulkCreate(alloctypeSchema, {
              updateOnDuplicate: [
                "status",
                "lname",
                "stamp",
      ...
      if (pushTable.indexOf("sposcombine") > -1 || pushTable.length == 0) {
        // sposcombine
        let sposcombineSchema = [];
        do {
          stamp = "";
          stamp = await Sposcombine.max("stamp", {
            where: { status: { [Op.or]: ["", " ", null] } },
          });
          if (stamp === null || stamp === 0) {
            stamp = "0x0000000000000000";
          }
          const sposcombine = await syncApi().callApi(
            "sposcombine",
            stamp,
            currentStampdata,
            tokenUsed
          ...
          if (sposcombine.sposcombine) {
            ...
            Sposcombine.bulkCreate(sposcombineSchema, {
              updateOnDuplicate: ["mixed", "status", "spos1", "spos2", "stamp"],
            }).then(() => {
